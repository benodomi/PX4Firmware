...
{
    double a = MAX((ceil( ( ( 2 * minRxSymbols - 8 ) * tSymbol + 2 * rxError ) / tSymbol )), minRxSymbols ); // Computed number of symbols
    *windowTimeout = (uint32_t) a;
    double b = ceil( ( 4.0 * tSymbol ) - ( ( *windowTimeout * tSymbol ) / 2.0 ) - wakeUpTime );
    *windowOffset = (int32_t) b;
}
...
int8_t RegionCommonComputeTxPower( int8_t txPowerIndex, float maxEirp, float antennaGain )
{
    ...
    double a = floor( ( maxEirp - ( txPowerIndex * 2U ) ) - antennaGain );
    phyTxPower = ( int8_t ) a;
