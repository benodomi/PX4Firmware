...
MAVLINK_MSG_ID_ROTOR_FREQUENCY = 255
...
class MAVLink_rotor_frequency_message(MAVLink_message):
        '''
        Send a rotor_frequency value. The index is used to discriminate between
        values. These values show up in the plot of QGroundControl as
        DEBUG N.
        '''
        def __init__(self, time_boot_ms, ind, value):
                MAVLink_message.__init__(self, MAVLINK_MSG_ID_ROTOR_FREQUENCY, 'ROTOR_FREQUENCY')
                self._fieldnames = ['indicated_frequency_hz', 'estimated_error_hz', 'indicated_frequency_rpm', 'estimated_error_rpm', 'count']
                self.indicated_frequency_rpm = indicated_frequency_rpm
                self.estimated_error_rpm  = estimated_error_rpm
                self.indicated_frequency_hz = indicated_frequency_hz
                self.estimated_error_hz  = estimated_error_hz
                self.count  = count;

        def pack(self, mav):
                return MAVLink_message.pack(self, mav, 255, struct.pack('<fffff', self.indicated_frequency_rpm, self.estimated_error_rpm, self.indicated_frequency_hz, self.estimated_error_hz, self.count))
...
mavlink_map = {
        ...
        MAVLINK_MSG_ID_ROTOR_FREQUENCY : ('<fffff', MAVLink_rotor_frequency_message, [0, 1, 2, 2, 4], 255)
}
